...
def main():
 ...
 report={'status':'PASS','scope':'Catalog content/schema/arithmetic only; current implementation and browser results are tracked separately in STATUS.md','checks':checks,'levels':len(levels),'outcomes':len(outcomes),'chapters':10,'levelsPerChapter':20,'templates':dict(weights),'canonicalAdaptations':sum(l['provenance']['kind']=='canonical-adaptation' for l in levels),'originalScenarios':sum(l['provenance']['kind']=='original' for l in levels),'epistemicFixedLevels':[l['id'] for l in levels if l['knowledge']['mode']=='epistemic-fixed'],'catalogSha256':digest,'openingFixture':{'rawDeaths':2,'weightedImpactTenths':3,'scoreExact':'250/3','scoreDisplay':'83.3'},'limitations':['This report does not execute the game or browser tests; see STATUS.md for current implementation evidence.','Scores are authored judgments, not empirically validated morality.','Narrative/causal review is editorial; integer consistency alone cannot prove philosophy or prose correct.']}
